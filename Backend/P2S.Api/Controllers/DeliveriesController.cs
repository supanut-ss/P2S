...
public class DeliveriesController : ControllerBase
{
    ...
    public async Task<ActionResult<List<GoodsReceiptOrderResponse>>> LookupOrder([FromQuery] string? orderNo, CancellationToken ct)
    {
        ...
        return Ok(orders.Select(order => new GoodsReceiptOrderResponse(
            ...
            OrderHeaderMetadataResolver.Resolve(order.PackageName, order.OrderItems.Select(item => item.PackageName)),
            OrderHeaderMetadataResolver.Resolve(order.ShopName, order.OrderItems.Select(item => item.ShopName)),
            OrderHeaderMetadataResolver.Resolve(order.TrackingNo, order.OrderItems.Select(item => item.TrackingNo)),
            OrderHeaderMetadataResolver.Resolve(order.Courier, order.OrderItems.Select(item => item.Courier)),
            order.OrderedAt,
